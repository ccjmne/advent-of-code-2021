import { isNotNil, type Definitely, type Maybe } from './maybe'
...
export function rangeBounds(from: number, to: number, step = 1): number[] {
  return range(Math.abs(to - from) + 1, { offset: from, step: Math.sign(to - from) * Math.abs(step) })
}
...
export function mul<T>(items: T[], by: (item: T) => number): number
export function mul<T extends number>(items: T[], by?: (item: T) => number): number
export function mul<T>(items: T[], by: (item: T) => number = identity): number {
  return items.reduce((total, item) => total * by(item), 1)
}
...
export function min<T, V>(values: [], by?: (item: T) => V): undefined
export function min<T, V>(values: [T, ...T[]], by?: (item: T) => V): Definitely<T>
export function min<T, V>(values: T[], by?: (item: T) => V): Maybe<T>
export function min<T, V>(values: T[], by: (item: T) => V = identity): Maybe<T> {
  return select(values, (a, b) => (by(b) < by(a) ? b : a))
}

export function max<T, V>(values: [], by?: (item: T) => V): undefined
export function max<T, V>(values: [T, ...T[]], by?: (item: T) => V): Definitely<T>
export function max<T, V>(values: T[], by?: (item: T) => V): Maybe<T>
export function max<T, V>(values: T[], by: (item: T) => V = identity): Maybe<T> {
  return select(values, (a, b) => (by(b) > by(a) ? b : a))
}
...
/**
 * Only maps when still searching.
 * Avoids using `items.map(...).find(...)` when the map operation is extremely pricey.
 */
...
export function pairs<T>(items: T[], loop = false): Array<[T, T]> {
  const pairings = [...items.slice(1), items[0]]
  return items.slice(0, loop ? items.length : items.length - 1).map((item, i) => [item, pairings[i]])
}
